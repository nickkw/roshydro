...
void turn(double angle){
  ...
  if(abs(th - init_angle) > abs(angle)){ // Turn is complete!
    ...
    if(angle > 0) {  // Left turn
      a_vel = a_vel_max_ * 0.6;
    }
    else if (angle < 0) { // Right turn
      a_vel = -1 * a_vel_max_ * 0.6;
    }
  }
...
void avoidObstacle2(){
  ...
  if(!avoiding){
    ...
    if(turnedDir == LEFT){
      angle = M_PI/2;
      ROS_INFO("FIRST ENCOUNTER! Guessing to turn left!");
    }
    else if(turnedDir == RIGHT){
      angle = -M_PI/2;
      ROS_INFO("FIRST ENCOUNTER! Guessing to turn right!");
    }
    turn(angle);
    avoiding = true;
  }
  ...
  if(turning){
    if(turnedDir == LEFT){
      angle = M_PI/2;
      ROS_INFO("Completing left turn...");
    }
    else if(turnedDir == RIGHT){
      angle = -M_PI/2;
      ROS_INFO("Completing right turn...");
    }
    else if(turnedDir == FLIP){
      angle = M_PI;
      ROS_INFO("Completing FLIP...");
    }
    turn(angle);
  }
  ...
  // Otherwise drive forward and avoid obstacles on either side.
    ...
    ROS_INFO("Continuing forward...");

    l_vel = fmin(fmin(range_f, range_f_l),range_f_r); // Drive forward

    if (range_l < 0.35 || range_r < 0.35) { // If the side sensors is a little close to the obstacle, turn.
      // l_vel = 0.4; // Give a slight bit of forward velocity
    ...
    if (range_f < 0.30 && turning == false) {
      ROS_INFO("OBSTACLE IN FRONT AGAIN! FLIPPING!");
      turning = false;
      angle = M_PI;
      turnedDir = FLIP;
      turn(angle);
...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    else if((noObstacleCounter < 30) && ( (range_f < range_thresh_ - 0.20) || (range_f_l < range_thresh_ - 0.20) || (range_f_r < range_thresh_ - 0.20) )) {
      noObstacleCounter--;
      noObstacleCounter = fmax(noObstacleCounter, 0);
      avoidObstacle2();

    }

    // if all sensor ranges > range_thresh_ cm, move on
    else if((noObstacleCounter >= 30) && (range_f > range_thresh_) && (range_f_l > range_thresh_) && (range_f_r > range_thresh_) && (range_l > range_thresh_) && (range_r > range_thresh_)) {
      ...
      noObstacleCounter++;
      noObstacleCounter = fmin(noObstacleCounter, 40);
      ROS_INFO("Waiting for no obstacles...");
      // std::stringstream tt;
      // tt << "0.222";
      // tt << " ";
      // tt << "0";
