...
Bug-like pathing algorithm. Takes in range messages from 5 ultrasound sensors placed on the
left, front left, front, front right, and right side of the robots and will avoid obstacles
as they come within a threshold.
...
// Updates reported ultrasound sensor ranges. Note that all ultrasound range messages are published to the /range topic, but sensors are differentiated by the message headers.
void rangeCallback(const sensor_msgs::Range::ConstPtr& msg)
{
  if(msg->header.frame_id == left_sensor_frame_)
    ...
    ROS_ERROR_ONCE("Unknown sensor header frame detected! Ensure that only the sensors that are being used are launched.");
}
...
// Guesses the initial direction to turn based on which bearing the waypoint is from the current position
int guessDirection(){
  int direction;
  double angle;
  angle = atan2((waypoint.point.y - y), (waypoint.point.x - x));
...
// Rotate the robot by angle amount. Needs fixing, code implementation doesn't work well.
...
int main(int argc, char **argv)
{
  ...
  // pnh_.param<int>("dummy", dummy_, 1); // Maximum linear velocity
  ...
  // Check avoid obstacle case first
    else if((range_f < range_thresh_) || (range_f_l < range_thresh_) || (range_f_r < range_thresh_) || (range_l < range_thresh_-0.25) || (range_r < range_thresh_-0.25)) {
      obstacleTimeCount = 0;
      avoidObstacle();
    }
  // if all sensor ranges > range_thresh_ cm, move on
    else if((range_f > range_thresh_) && (range_f_l > range_thresh_) && (range_f_r > range_thresh_) && (range_l > range_thresh_-0.25) && (range_r > range_thresh_-0.25)) {
      if (obstacleTimeCount > 0) {
